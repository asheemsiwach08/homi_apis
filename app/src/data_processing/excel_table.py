...
def filter_and_deduplicate_csv(df):
    ...
    if not duplicates.empty:
        logger.info(f"Found {len(duplicates)} duplicate records based on bankAppId")
    ...
    logger.info(f"Deduplication complete. Kept {len(final_df)} records out of {len(df)} original records")
    logger.info(f"Final dataframe has {len(final_df.columns)} columns: {list(final_df.columns)}")
